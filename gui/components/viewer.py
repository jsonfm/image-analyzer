...
class Viewer:
    ...
    def setImage(self, img=None, a=False):      
        ...
        # image = img.copy()
        # if image.dtype != 'uint8' and a:
        #     image = image.astype('uint8')
        # data = image.data
        image = normalizeArray(img)
        data = image.data
        qt_img = QImage(data, image.shape[0], image.shape[1], image.strides[0], QImage.Format_Grayscale16)
        qt_img = qt_img.rgbSwapped()
        pixmap = QPixmap.fromImage(qt_img)
        #pixmap = arrayToPixmap(img, w, h, linewidth)
        if self.hasImage():
    ...
    def toggleDragMode(self):
        ...
        elif not self._pixmapHandle.pixmap().isNull():
